...
本仓库**本身就是 ComfyUI 插件**（custom node）：克隆/下载后，把整个仓库放入
`ComfyUI/custom_nodes/ArtMirror`，重启 ComfyUI 即可 —— 侧边栏出现「图库」tab，
可在 ComfyUI 内直接浏览/管理输出图片（后端随 ComfyUI 进程内启动，数据落 `user/artmirror/`）。

![ComfyUI 插件-侧边栏图库 tab](docs/screenshots/plugin.png)

```bash
cd ComfyUI/custom_nodes
git clone https://github.com/ShenZiLi/ArtMirror.git ArtMirror
# 或：下载 zip 解压为 custom_nodes/ArtMirror
# 重启 ComfyUI —— 依赖由 requirements.txt 自动安装（首次约 1-3 分钟）
```

- **解压即用**：仓库根含插件入口 `__init__.py`、侧边栏扩展 `web/`、依赖清单 `requirements.txt`，无任何子目录挑选
- **依赖**：ComfyUI 启动时按 `requirements.txt` 自动安装（标准机制，无需手动 pip）
- **数据**：插件场景落 `ComfyUI/user/artmirror/`；web 独立版落仓库 `data/`，互不影响
- **分发给小白**：可运行 `python scripts/build_plugin.py` 生成单文件 zip（2.5MB，解压即用）
...
```
ArtMirror/                       # 仓库根即 ComfyUI 插件：clone 后放入 custom_nodes/ArtMirror 解压即用
├── __init__.py                  # 插件入口：NODE_CLASS_MAPPINGS / WEB_DIRECTORY / 路由注册
├── web/artmirror-tab.js         # 侧边栏「图库」tab 扩展（WEB_DIRECTORY）
...
├── proxy.py                     # 反向代理（透传 query string）
├── comfy_paths.py               # ComfyUI 路径解析（user/output 目录）
├── frontend/                    # 前端（唯一一份，零构建；web 端与插件端共用）
│   ├── gallery.html / settings.html / image.html / index.html
│   ├── api.js / app.js / style.css / mock-data.js
│   └── vendor/alpine.min.js
├── launchers/web/main.py        # web 端辅助启动器（uvicorn :8000 + data/）
├── scripts/build_plugin.py      # 打包单文件 zip（分发给小白）
├── tests/                       # pytest（核心 tests/ + 插件集成 tests/plugins/）
├── pyproject.toml               # uv 管理依赖（editable 安装 artmirror）
└── docs/                        # 功能清单 / 设计文档 / 截图
```

> 双端关系（插件端为主，web 端为辅）：核心代码唯一真源在 `artmirror/` + `frontend/`，
> **改代码即双端生效，无需任何构建/同步**。插件场景（ComfyUI 进程内）与 web 场景
> （`launchers/web/main.py`）通过 `create_app()` 注入不同 data_dir 区分。

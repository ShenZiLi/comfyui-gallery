...
- pytest 新增：
  - `config` 冻结态路径计算（monkeypatch `sys.frozen`/`sys.executable`/`_MEIPASS` → 断言 data_dir 指向 exe 旁、frontend_dir 指向 _MEIPASS/frontend）
...
| 冻结态路径算错致数据丢失 | 单测覆盖；data_dir 只依赖 exe 位置，稳定 |

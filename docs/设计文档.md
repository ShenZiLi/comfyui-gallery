...
| `ImageAsset`        | 图片资产（只存引用）              | folder\_id、file\_name、file\_path（相对根）、abs\_path（物理绝对路径）、path\_key（规范化物理路径，活动资产唯一）、sha256（内容缓存键）、width/height/file\_size/file\_mtime、thumb\_ok、prompt\_type、rating（人工 1-5）、ai\_rating（AI 0-100）                                                                         |
...
* `init_db()`：建表 + 幂等迁移；旧图片表先以 SQLite backup 备份，再迁移到路径键身份并补齐解析器版本列；保留资产 ID、评分及关联记录
...
* `POST /roots` 注册扫描目录并返回扫描任务

* `POST /scans` 手动启动所有已注册目录扫描；`GET /scans/active` 恢复活动任务；`GET /scans/{task_id}` 轮询进度、结果和错误
...
* `scan_all / scan`：递归扫描根目录，按规范化物理路径识别资产；仅新增或变化文件读取、哈希和解析，缩略图按 SHA-256 共享缓存

* 嵌套根处理：注册在父根之下的嵌套根，其子树由嵌套根自己扫描（父根跳过，避免重复）

* 单次目录遍历建立 Folder 树并记录图片元信息；以 50 张为一批提交，单图错误隔离

* 软删：库中在根内但本次未扫描到的文件标记 `is_deleted=1`

* `scan_tasks`：添加、导入、手动与定时扫描共用单进程任务协调器；任务 API 提供状态与真实文件进度

* `unlink_root`：移除根时软删其下图片并清理目录节点

#### watcher.py — 后台实时同步

* 常驻线程 `_loop`：每 20s 请求协调器扫描注册根；协调器忙碌时跳过本轮，有增删改则递增版本号

* 扫描协调器串行运行手动、后台和定时任务；定时请求在忙碌时跳过
...
```
注册根目录（设置页 / 导入上传）
  → scan_tasks 合并重复根并串行调度
  → scanner.scan(根)：os.walk 递归并统计文件
    → 跳过隐藏目录与嵌套根子树
    → 图片文件：路径键定位 + mtime/size 增量；SHA-256 仅作内容缓存键
    → 读取字节 → comfyui_parser.parse_bytes（解析 workflow/prompt）
    → 复用或生成 WebP 缩略图（data/thumbs/<sha>.webp）
    → 建 Folder 树 → 新建/更新 ImageAsset
    → meta_service.ingest：写 WorkflowMeta + 标签
  → 软删消失文件
  → 任务状态 API 更新文件进度；完成后 watcher.bump()
  → 前端轮询任务状态展示进度，pollSync（3s）检测图库版本变化
```
